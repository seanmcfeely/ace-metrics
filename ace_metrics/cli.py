...
from .alerts import VALID_ALERT_STATS, FRIENDLY_STAT_NAME_MAP, ALERTS_BY_MONTH_DB_QUERY, statistics_by_month_by_dispo
...
def build_metric_user_parser(user_parser: argparse.ArgumentParser) -> None:
    ...
    user_parser.add_argument("-l", "--list-users", action="store_true", help="List all users")
    ...
    user_parser.add_argument(
        "-tac",
        "--total-alert-count-breakdown",
        action="store_true",
        help="Total count of Alerts worked by analyst by month.",
    )

    for stat in VALID_ALERT_STATS:
        user_parser.add_argument(
            f"--{stat}", action="store_true", dest=f"user_stat_{stat}", help=FRIENDLY_STAT_NAME_MAP[stat]
        )
    user_parser.add_argument(
        "--all-stats", dest="all_user_stats", action="store_true", help="Return all of the available statistics."
    )
...
def build_metric_alert_type_parser(alert_type_parser: argparse.ArgumentParser) -> None:
    ...
    alert_type_parser.add_argument("-l", "--list-alert-types", action="store_true", help="List the types of alerts")
    alert_type_parser.add_argument(
        "-t",
        "--type",
        action="append",
        dest="types",
    ...
    for stat in VALID_ALERT_STATS:
        alert_type_parser.add_argument(
            f"--{stat}", action="store_true", dest=f"alert_type_stat_{stat}", help=FRIENDLY_STAT_NAME_MAP[stat]
        )
    alert_type_parser.add_argument(
        "--all-stats", dest="all_at_stats", action="store_true", help="Return all of the available statistics."
    )
...
def build_metric_alert_parser(alert_parser: argparse.ArgumentParser) -> None:
    ...
    alert_parser.add_argument(
        "-hop", "--hours-of-operation", action="store_true", help='Generate "Hours of Operation" summary by month'
    )
    alert_parser.add_argument(
        "-avg-ct-sum",
        "--average-alert-cycletime-summary",
        action="store_true",
    ...
    for stat in VALID_ALERT_STATS:
        alert_parser.add_argument(
            f"--{stat}", action="store_true", dest=f"alert_stat_{stat}", help=FRIENDLY_STAT_NAME_MAP[stat]
        )
    alert_parser.add_argument(
        "--all-stats", dest="all_alert_stats", action="store_true", help="Return all of the available statistics."
    )
...
def build_metric_event_parser(event_parser: argparse.ArgumentParser) -> None:
    ...
    event_parser.add_argument("-i", "--incidents", action="store_true", help="Create incident table from event table")
    event_parser.add_argument(
        "-io", "--incidents-only", action="store_true", help="Only return incidents table (skip event table)"
    )
    event_parser.add_argument(
        "-ce", "--count-emails", action="store_true", help="Count emails, in each event, per company."
    )
...
def build_metric_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        action="store",
        choices=FILEOUT_FORMATS,
        help="desired file output format. Default is xls.",
    )
    parser.add_argument(
        "-f", "--filename", action="store", default=None, help="The name of a file to write results to."
    )
    parser.add_argument(
        "-c",
        "--company",
    ...
    event_parser = metrics_subparsers.add_parser(
        "events", help="event based metrics. With no arguments will return all events"
    )
...
def execute_metric_arguments(db: pymysql.connections.Connection, args):
    ...
    from .alerts.users import get_all_users, generate_user_alert_stats, alert_quantities_by_user_by_month
    ...
    from .events import get_events_between_dates, get_incidents_from_events, add_email_alert_counts_per_event

    from .helpers import get_companies, dataframes_to_archive_bytes_of_json_files, dataframes_to_xlsx_bytes
    ...
    if args.metric_target == "alerts":
        ...
        if args.alert_metric_target == "types":
            ...
            at_arg_stats = [
                sarg[len("alert_type_stat_") :]
                for sarg, value in vars(args).items()
                if sarg.startswith("alert_type_stat_") and value is True
            ]
            ...
            if args.list_alert_types or args.all_at_stats or at_arg_stats:
                ...
                if args.all_at_stats or at_arg_stats:
                    start_date = datetime.datetime.strptime(args.start_datetime, "%Y-%m-%d %H:%M:%S")
                    end_date = datetime.datetime.strptime(args.end_datetime, "%Y-%m-%d %H:%M:%S")
                    alert_type_map = get_alerts_between_dates_by_type(
                        start_date, end_date, db, selected_companies=args.companies
                    )
        ...
        alert_arg_stats = [
            sarg[len("alert_stat_") :]
            for sarg, value in vars(args).items()
            if sarg.startswith("alert_stat_") and value is True
        ]

        # only get alert data if we really needs it
        if (
            args.alert_metric_target == "users"
            or alert_arg_stats
            or args.all_alert_stats
            or args.hours_of_operation
            or args.average_alert_cycletime_summary
        ):
            start_date = datetime.datetime.strptime(args.start_datetime, "%Y-%m-%d %H:%M:%S")
            end_date = datetime.datetime.strptime(args.end_datetime, "%Y-%m-%d %H:%M:%S")
            ...
            for user_id in user_ids:
                if args.all_user_stats:
                    ...
                    user_arg_stats = [
                        sarg[len("user_stat_") :]
                        for sarg, value in vars(args).items()
                        if sarg.startswith("user_stat_") and value is True
                    ]
